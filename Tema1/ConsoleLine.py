        ...
        for neighbour in get_neighbours(current_position):
            if is_valid_transition(state, neighbour):
                if is_final_state(state, neighbour):
                    ...
                    return path
                succ = neighbour
                if is_better(succ, current_position, end):
...
def simulated_solve_maze( state, start, end):
    current_position = start
    new_transition(state, start)
    path = [current_position]
    while current_position != end:
        prev_state = current_position
        for neighbour in get_neighbours(current_position):
            if is_valid_transition(state, neighbour):
                ...
                if is_better(succ, current_position, end):
                    current_position = succ
                elif uniform(0, 1) < np.exp(
                        -abs(value(current_position, end) - value(succ, end)) / temperature):
                    current_position = succ
        if current_position == prev_state:
            return False
...
if __name__ == '__main__':
    ...
    while answer != 'quit':
        ...
        if answer == '1':
            ...
            print(simulated_solve_maze(initial_state, start, end))
        else:
